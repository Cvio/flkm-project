...
contract DataAnalysisContract is Initializable, AccessControlUpgradeable, ChainlinkClient {
    ...
    event AnalysisRequested(bytes32 requestId, string datasetId, string datasetUrl);
    ...
    function fulfill(bytes32 _requestId, string memory _result) public recordChainlinkFulfillment(_requestId) onlyRole(ANALYST_ROLE) {
        analysisResults[_requestId] = _result;
        emit AnalysisFulfilled(_requestId, _result);
    }

    function storeIpfsHash(bytes32 datasetId, string memory ipfsHash) public onlyRole(ANALYST_ROLE) {
        require(bytes(ipfsHash).length > 0, "Invalid IPFS Hash");
        datasetIpfsHashes[datasetId] = ipfsHash;
        emit IpfsHashStored(datasetId, ipfsHash);
    }

    function getIpfsHash(bytes32 datasetId) public view returns (string memory) {
        require(bytes(datasetIpfsHashes[datasetId]).length > 0, "IPFS Hash not found");

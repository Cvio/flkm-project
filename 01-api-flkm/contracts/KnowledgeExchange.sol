...
contract KnowledgeExchangeContract is Ownable, ReentrancyGuard {
    ...
    enum ReputationLevel { Mystic, Luminary, Oracle, Prophet }
    ...
    function uploadResource(string memory _title, string memory _description) external {
    ...
    function accessResource(uint256 _resourceID) external nonReentrant {
        ...
        knowledgeToken.transferFrom(msg.sender, knowledgeResources[_resourceID].creator, compensationAmount);

        emit ResourceAccessed(_resourceID, msg.sender);
    }

    function calculateCompensation(uint256 _resourceID) internal view returns (uint256) {
        require(knowledgeResources[_resourceID].exists, "Resource does not exist");

        uint256 demand = knowledgeResources[_resourceID].demand;
        uint256 reputationBonus = reputationBonusValues[userReputation[msg.sender]];
        uint256 uniquenessFactor = MAX_DEMAND.sub(demand).mul(UNIQUENESS_SCALING_FACTOR);
        uint256 contributionBonus = userResourceCount[msg.sender].mul(CONTRIBUTION_SCALING_FACTOR).add(userTotalDemand[msg.sender]);

        uint256 compensation = demand.add(reputationBonus).add(uniquenessFactor).add(contributionBonus);
        return (compensation < MINIMUM_COMPENSATION) ? MINIMUM_COMPENSATION : compensation;
    }

    function updateUserReputation(address _user, ReputationLevel _newLevel) external onlyOwner {
        require(userReputation[_user] < _newLevel, "Cannot decrease reputation level");
        userReputation[_user] = _newLevel;
    }

    function updateReputationBonusValue(ReputationLevel _level, uint256 _value) external onlyOwner {
